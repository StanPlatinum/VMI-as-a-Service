                         ...
                         int total_size)
{
    ...
    {
	/* inserting a parser for VMFUNC... */
	//printf("starting address = %llx\n", (unsigned long long)start);
	//printf("size = %d\n", size);
	printf("the  5th char after the start point is : %u\n", *(start + 4));
	printf("the  9th char after the start point is : %u\n", *(start + 8));

	_rv = system("/home/whu-vmfunc/cheng/process+socket/sock_vmi/sock_file_list");
