...
void hvm_rdtsc_intercept(struct cpu_user_regs *regs)
{
    ...
    /* tracing vmfunc */
    if (eptp_idx != eptp_idx_last) {
	HVMTRACE_2D(VMFUNC, (unsigned long)curr->domain->domain_id, (unsigned long)eptp_idx);
	eptp_idx_last = eptp_idx;
    }
...
long do_hvm_op(unsigned long op, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    long rc = 0;
/*
printk("entering do_hvm_op and op = %lu...\n", op);
*/
...
printk("entering get_time...\n");

        gxt.now = NOW();

printk("NOW  = %llu\n", (unsigned long long)gxt.now);
...
int do_disable_intercept_rdtsc(void)
{
    ...
    eptp_idx = 0;	/* maybe needs some RCUs... */
